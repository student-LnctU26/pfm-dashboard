...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || data.meassage || "Registration failed");
      }
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-slate-850 p-8 rounded-xl border border-slate-750 shadow-lg space-y-6">
        <div className="text-center">
          ...
          <p className="text-slate-400 text-sm mt-2">Create your PFM account</p>
        </div>
        ...
        {success && (
          <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-sm p-3 rounded-lg text-center">
            {success}
          </div>
        )}
